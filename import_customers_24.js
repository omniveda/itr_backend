...
const CSV_FILE_PATH = 'F:/Project/Incometaxreturn/Data/real_data/customer-24-real-data.csv';
...
function fixSciNotation(val) {
    ...
    if (s.includes('E+') || s.includes('e+')) {
        return Number(s).toLocaleString('fullwide', { useGrouping: false }).replace(/\.0+$/, '');
    }
    return s;
}

async function importCustomers24() {
    try {
        await initDb();
        ...
        fs.createReadStream(CSV_FILE_PATH)
            ...
            .on('end', async () => {
                ...
                const cleanNum = (val) => {
                    ...
                    // Range check for decimal(10,2): -99999999.99 to 99999999.99
                    const num = parseFloat(s);
                    if (num > 99999999.99 || num < -99999999.99) return null;
                    return s;
                };
                ...
                for (const customer of customers) {
                    try {
                        ...
                        const customerData = {
                            ...
                            attachments_6: (customer.attachments_6 || customer.attachment_6 || '').substring(0, 255)
                        };
